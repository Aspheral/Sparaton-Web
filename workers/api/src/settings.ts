...
export async function adminSettings(request:Request,env:Env):Promise<Response>{
  ...
  if(request.method==='GET'){
    const rows=await env.DB.prepare('SELECT key,value_json,updated_at,updated_by FROM site_settings ORDER BY key').all();
  ...
  await env.DB.batch([
    env.DB.prepare('INSERT INTO site_settings(key,value_json,updated_by,updated_at) VALUES(?1,?2,?3,CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value_json=excluded.value_json,updated_by=excluded.updated_by,updated_at=CURRENT_TIMESTAMP').bind(key,valueJson,admin.email),
