...
function parseRepository(url:string){try{const parsed=new URL(url);if(parsed.hostname.toLowerCase()!=='github.com')return null;const parts=parsed.pathname.replace(/\.git$/,'').split('/').filter(Boolean);if(parts.length!==2)return null;const [owner,repo]=parts;if(!owner||!repo)return null;return `${encodeURIComponent(owner)}/${encodeURIComponent(repo)}`;}catch{return null;}}
