...
import { notifyAssignedStaff } from './notifications';
import { hashToken, randomId, randomToken, sha256 } from './security';

const SESSION_COOKIE='sparaton_ticket';
...
export async function createTicket(request:Request,env:Env):Promise<Response>{
  ...
  const name=clean(data.name,100,true),email=normalizeEmail(clean(data.email,254,true)),inquiryType=clean(data.inquiryType,60,true),subject=clean(data.subject,180,true),message=clean(data.message,12000,true),organization=clean(data.organization,160,false),budget=clean(data.budgetRange,120,false),preferredTeam=clean(data.preferredTeam,120,false);
  ...
  if(existing){await sendVerification(env,request,{ticketId:existing.id,email,name:existing.requester_name},`existing-${Date.now()}`);return respond(request,409);}
  const ticketId=randomId('tkt'),publicId=randomToken(16),messageId=randomId('msg');
  await env.DB.batch([
    env.DB.prepare("INSERT INTO tickets (id,public_id,requester_email_normalized,requester_name,requester_organization,inquiry_type,subject,preferred_team,budget_range,status,last_response_at) VALUES (?1,?2,?3,?4,?5,?6,?7,?8,?9,'new',CURRENT_TIMESTAMP)").bind(ticketId,publicId,email,name,organization||null,inquiryType,subject,preferredTeam||null,budget||null),
    env.DB.prepare("INSERT INTO ticket_messages (id,ticket_id,author_kind,body,safe_email_preview) VALUES (?1,?2,'client',?3,?4)").bind(messageId,ticketId,message,safePreview(message)),
  ...
  try{await sendVerification(env,request,{ticketId,email,name},`new-${ticketId}`);}catch(error){
    await env.DB.batch([
...
export async function verifyTicket(request:Request,env:Env):Promise<Response>{
  const raw=new URL(request.url).searchParams.get('token');if(!raw)throw new HttpError(400,'Missing verification token');
  ...
  const sessionToken=randomToken(),sessionHash=await hashToken(sessionToken,env.SESSION_SIGNING_SECRET),sessionId=randomId('ses'),expires=new Date(Date.now()+SESSION_MAX_AGE_SECONDS*1000).toISOString();
  await env.DB.batch([
...
export async function getTicket(request:Request,env:Env,publicId:string):Promise<Response>{
  const auth=await authorizeTicket(request,env,publicId),ticket=await env.DB.prepare('SELECT public_id,requester_name,inquiry_type,subject,status,priority,created_at,updated_at FROM tickets WHERE id=?1').bind(auth.ticketId).first(),messages=await env.DB.prepare('SELECT id,author_kind,body,created_at,edited_at FROM ticket_messages WHERE ticket_id=?1 ORDER BY created_at,id').bind(auth.ticketId).all();
  await env.DB.batch([
    env.DB.prepare('UPDATE ticket_access_sessions SET last_seen_at=CURRENT_TIMESTAMP WHERE id=?1').bind(auth.sessionId),
    env.DB.prepare("UPDATE ticket_participants SET unread_count=0,last_read_at=CURRENT_TIMESTAMP WHERE ticket_id=?1 AND kind='client'").bind(auth.ticketId)
  ]);
  return json({ticket,messages:messages.results,csrf:auth.csrf});
}

export async function postClientMessage(request:Request,env:Env,publicId:string):Promise<Response>{
  const auth=await authorizeTicket(request,env,publicId);if(request.headers.get('x-sparaton-csrf')!==auth.csrf)throw new HttpError(403,'Invalid request token');
  const data=await request.json<{body?:unknown}>(),body=clean(data.body,12000,true),ticket=await env.DB.prepare('SELECT status,subject,requester_name FROM tickets WHERE id=?1').bind(auth.ticketId).first<{status:string;subject:string;requester_name:string}>();
  if(!ticket)throw new HttpError(404,'Ticket not found');if(['closed','archived'].includes(ticket.status))throw new HttpError(409,'This ticket is closed');
  const messageId=randomId('msg'),previous=ticket.status;
  await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO ticket_status_events(id,ticket_id,previous_status,next_status,actor_kind,actor_id) VALUES(?1,?2,?3,'awaiting_staff','client',NULL)").bind(randomId('evt'),auth.ticketId,previous),
  ...
  const online=await (await room.fetch('https://room/presence')).json<{staff:number}>();
  await notifyAssignedStaff(env,{ticketId:auth.ticketId,publicId,messageId,subject:ticket.subject,preview:safePreview(body),requesterName:ticket.requester_name,staffOnline:online.staff>0});
...
export async function connectTicketSocket(request:Request,env:Env,publicId:string):Promise<Response>{
  const auth=await authorizeTicket(request,env,publicId),room=env.TICKET_ROOMS.get(env.TICKET_ROOMS.idFromName(publicId)),headers=new Headers(request.headers);headers.set('x-sparaton-role','client');headers.set('x-sparaton-participant',auth.sessionId);return room.fetch(new Request('https://room/connect',{headers}));
}
...
export async function authorizeTicket(request:Request,env:Env,publicId:string){
  ...
  const hash=await hashToken(raw,env.SESSION_SIGNING_SECRET),row=await env.DB.prepare('SELECT s.id AS session_id,s.ticket_id FROM ticket_access_sessions s JOIN tickets t ON t.id=s.ticket_id WHERE s.token_hash=?1 AND t.public_id=?2 AND s.revoked_at IS NULL AND s.expires_at>CURRENT_TIMESTAMP').bind(hash,publicId).first<{session_id:string;ticket_id:string}>();
  if(!row)throw new HttpError(401,'Ticket session is invalid or expired');return{sessionId:row.session_id,ticketId:row.ticket_id,csrf:await hashToken(`csrf:${row.session_id}`,env.SESSION_SIGNING_SECRET)};
}

async function enforceRateLimit(request:Request,env:Env,email:string){const ip=request.headers.get('cf-connecting-ip')??'unknown',key=await sha256(`${ip}:${email}`),since=new Date(Date.now()-15*60*1000).toISOString(),row=await env.DB.prepare("SELECT COUNT(*) AS count FROM rate_limit_events WHERE key_hash=?1 AND bucket='ticket-create' AND occurred_at>=?2").bind(key,since).first<{count:number}>();if((row?.count??0)>=5)throw new HttpError(429,'Too many attempts. Try again later.');await env.DB.prepare("INSERT INTO rate_limit_events (key_hash,bucket) VALUES (?1,'ticket-create')").bind(key).run();}
...
function respond(request:Request,status:number){if((request.headers.get('accept')??'').includes('text/html'))return new Response(`<main style="font-family:system-ui;max-width:48rem;margin:10vh auto;padding:2rem"><h1>${status===201?'Check your email':'Existing conversation found'}</h1><p>${status===201?'We sent a secure verification link.':'We sent a fresh secure link to your existing ticket.'}</p><p><a href="https://sparaton.com">Return to Sparaton</a></p></main>`,{status,headers:{'content-type':'text/html; charset=utf-8'}});return json({verificationSent:true,existing:status===409},status);}
