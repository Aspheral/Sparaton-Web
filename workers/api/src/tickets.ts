...
export async function getTicket(request:Request,env:Env,publicId:string):Promise<Response>{
  ...
  const ticket=await env.DB.prepare('SELECT public_id,requester_name,inquiry_type,subject,status,priority,created_at,updated_at FROM tickets WHERE id=?1').bind(auth.ticketId).first();
  const messages=await env.DB.prepare('SELECT id,author_kind,body,created_at,edited_at FROM ticket_messages WHERE ticket_id=?1 ORDER BY created_at,id').bind(auth.ticketId).all();
  await env.DB.prepare('UPDATE ticket_access_sessions SET last_seen_at=CURRENT_TIMESTAMP WHERE id=?1').bind(auth.sessionId).run();
