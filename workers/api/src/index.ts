...
export default {
  async fetch(request:Request,env:Env):Promise<Response>{
    ...
    try{
      ...
      if(method==='GET'&&p==='/health')response=Response.json({ok:true});
      ...
      else if(method==='GET'&&/^\/v1\/admin\/(people|organizations|services|projects|posts|settings)$/.test(p))response=await adminCollection(request,env,p.split('/')[3]!);
      ...
      else if(method==='POST'&&p==='/v1/admin/settings')response=await upsertSetting(request,env);
      ...
      else response=new Response(JSON.stringify({error:'Not found'}),{status:404,headers:{'content-type':'application/json; charset=utf-8'}});
      return cors(withSecurity(response),request,env);
    }catch(error){
      ...
      console.error(error);
      return cors(withSecurity(Response.json({error:message},{status})),request,env);
    }
  }
};
