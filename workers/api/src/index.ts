...
export default {
  async fetch(request:Request,env:Env):Promise<Response>{
    ...
    try{
      const url=new URL(request.url); const p=url.pathname; let response:Response;
      ...
      for(const [key,value] of Object.entries(securityHeaders))response.headers.set(key,value); return cors(response,request,env);
    }catch(error){
